...
# 6 "/opt/gcc-arm-11.2-2022.02-x86_64-arm-none-eabi/arm-none-eabi/include/math.h" 2 3
...
# 7 "/opt/gcc-arm-11.2-2022.02-x86_64-arm-none-eabi/arm-none-eabi/include/math.h" 2 3
# 1 "/opt/gcc-arm-11.2-2022.02-x86_64-arm-none-eabi/arm-none-eabi/include/machine/ieeefp.h" 1 3
# 8 "/opt/gcc-arm-11.2-2022.02-x86_64-arm-none-eabi/arm-none-eabi/include/math.h" 2 3
# 1 "/opt/gcc-arm-11.2-2022.02-x86_64-arm-none-eabi/arm-none-eabi/include/_ansi.h" 1 3
# 9 "/opt/gcc-arm-11.2-2022.02-x86_64-arm-none-eabi/arm-none-eabi/include/math.h" 2 3
...
# 28 "stm/src/bitbang.c" 2
# 1 "stm/src/nvic.h" 1
# 17 "stm/src/nvic.h"
       
# 29 "stm/src/bitbang.c" 2
...
# 82 "stm/src/bitbang.h" 3 4
   _Bool 
# 82 "stm/src/bitbang.h"
        inputActive;
...
void bbGpioSetup(bbPort_t * bbPort, int pinIndex, IO_t io, uint8_t puPdMode)
{
    ...
    bbPort->gpioModeMask |= (((uint32_t)0x00000003) << (pinIndex * 2));
    bbPort->gpioModeInput |= (GPIO_Mode_IN << (pinIndex * 2));
    bbPort->gpioModeOutput |= (GPIO_Mode_OUT << (pinIndex * 2));

    bbPort->gpioIdleBSRR |= (1 << (pinIndex + 16));

    IOWrite(io, 0);

    IOConfigGPIO(io, ((GPIO_Mode_OUT) | ((GPIO_Fast_Speed) << 2) | ((GPIO_OType_PP) << 4) | ((puPdMode) << 5)));
}
...
void bbTimerChannelInit(bbPort_t *bbPort, resourceOwner_e owner)
{
    ...
    TIM_OCStruct.TIM_OCMode = ((uint16_t)0x0060);
    TIM_OCStruct.TIM_OCIdleState = ((uint16_t)0x0100);
    TIM_OCStruct.TIM_OutputState = ((uint16_t)0x0001);
    TIM_OCStruct.TIM_OCPolarity = ((uint16_t)0x0002);
...
void bbSwitchToOutput(bbPort_t * bbPort)
{
    ...
    ((bbPort->gpio->BSRRL) = (bbPort->gpioIdleBSRR));
    ...
    for ( uint8_t __basepri_save __attribute__ ((__cleanup__ (__basepriRestoreMem), __unused__)) = __get_BASEPRI(), __ToDo = __basepriSetMemRetVal((((((1)<<(4-(7-(((uint32_t)0x500)>>8))))|((1)&(0x0f>>(7-(((uint32_t)0x500)>>8)))))<<4)&0xf0)); __ToDo ; __ToDo = 0 ) {
        (((bbPort->gpio->MODER)) = ((((((bbPort->gpio->MODER))) & (~(bbPort->gpioModeMask))) | (bbPort->gpioModeOutput))));
    }
...
void bbDMAPreconfigure(bbPort_t *bbPort, bitbangDirection_e direction)
{
    ...
    dmainit->DMA_Mode = ((uint32_t)0x00000000);
    dmainit->DMA_Channel = bbPort->dmaChannel;
    dmainit->DMA_PeripheralInc = ((uint32_t)0x00000000);
    dmainit->DMA_MemoryInc = ((uint32_t)0x00000400);
    dmainit->DMA_FIFOMode = ((uint32_t)0x00000004) ;
    dmainit->DMA_FIFOThreshold = ((uint32_t)0x00000000) ;
    dmainit->DMA_MemoryBurst = ((uint32_t)0x00000000) ;
    dmainit->DMA_PeripheralBurst = ((uint32_t)0x00000000);

    if (direction == BITBANG_DIRECTION_OUTPUT) {
        dmainit->DMA_Priority = ((uint32_t)0x00020000);
        dmainit->DMA_DIR = ((uint32_t)0x00000040);
        dmainit->DMA_BufferSize = bbPort->portOutputCount;
        dmainit->DMA_PeripheralBaseAddr = (uint32_t)&bbPort->gpio->BSRRL;
        dmainit->DMA_PeripheralDataSize = ((uint32_t)0x00001000);
        dmainit->DMA_Memory0BaseAddr = (uint32_t)bbPort->portOutputBuffer;
        dmainit->DMA_MemoryDataSize = ((uint32_t)0x00004000);

        DMA_Init((DMA_Stream_TypeDef *)(bbPort->dmaResource), dmainit);
        bbSaveDMARegs(bbPort->dmaResource, &bbPort->dmaRegOutput);
    } else {
        dmainit->DMA_Priority = ((uint32_t)0x00030000);
        dmainit->DMA_DIR = ((uint32_t)0x00000000);
        dmainit->DMA_BufferSize = bbPort->portInputCount;

        dmainit->DMA_PeripheralBaseAddr = (uint32_t)&bbPort->gpio->IDR;

        dmainit->DMA_PeripheralDataSize = ((uint32_t)0x00000800);
...
void bbTIM_TimeBaseInit(bbPort_t *bbPort, uint16_t period)
{
    ...
    init->TIM_ClockDivision = ((uint16_t)0x0000);
    init->TIM_CounterMode = ((uint16_t)0x0000);
...
void bbDMA_ITConfig(bbPort_t *bbPort)
{
    DMA_ITConfig((DMA_Stream_TypeDef *)(bbPort->dmaResource), ((uint32_t)0x00000010), ENABLE);
}

void bbDMA_Cmd(bbPort_t *bbPort, FunctionalState NewState)
{
    DMA_Cmd((DMA_Stream_TypeDef *)(bbPort->dmaResource), NewState);
}

int bbDMA_Count(bbPort_t *bbPort)
{
    return DMA_GetCurrDataCounter((DMA_Stream_TypeDef *)(bbPort->dmaResource));
}
