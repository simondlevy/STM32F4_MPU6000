# 0 "stm/src/persistent.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "stm/src/persistent.c"
# 22 "stm/src/persistent.c"
...
void persistentObjectInit(void)
{
    ...
    wasSoftReset = ((RCC_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x3800))->CSR & ((uint32_t)0x10000000);

    if (!wasSoftReset || (persistentObjectRead(PERSISTENT_OBJECT_MAGIC) != (('B' << 24)|('e' << 16)|('f' << 8)|('1' << 0)))) {
        for (int i = 1; i < PERSISTENT_OBJECT_COUNT; i++) {
            persistentObjectWrite(i, 0);
        }
        persistentObjectWrite(PERSISTENT_OBJECT_MAGIC, (('B' << 24)|('e' << 16)|('f' << 8)|('1' << 0)));
    }
}
