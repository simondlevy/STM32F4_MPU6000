...
static 
# 76 "stm/src/spi.c" 3 4
      _Bool 
# 76 "stm/src/spi.c"
           isCcm(const uint8_t * p)
{
    return (((uint32_t)p & 0xffff0000) == 0x10000000);
}

static SPI_InitTypeDef defaultInit = {
    .SPI_Mode = ((uint16_t)0x0104),
    .SPI_Direction = ((uint16_t)0x0000),
    .SPI_DataSize = ((uint16_t)0x0000),
    .SPI_NSS = ((uint16_t)0x0200),
    .SPI_FirstBit = ((uint16_t)0x0000),
    .SPI_CRCPolynomial = 7,
    .SPI_BaudRatePrescaler = ((uint16_t)0x0010),
    .SPI_CPOL = ((uint16_t)0x0002),
    .SPI_CPHA = ((uint16_t)0x0001)
};


static 
# 94 "stm/src/spi.c" 3 4
      _Bool 
# 94 "stm/src/spi.c"
           spiIsBusy(const spiDevice_t *dev)
{
...
# 146 "stm/src/spi.c"
    ...
    if (dev->leadingEdge != bus->leadingEdge) {
        ...
        instance->CR1 &= ~(((uint16_t)0x0002) | ((uint16_t)0x0001));


        if (dev->leadingEdge) {
            instance->CR1 |= ((uint16_t)0x0000) | ((uint16_t)0x0000);
        } else
        {
            instance->CR1 |= ((uint16_t)0x0002) | ((uint16_t)0x0001);
        }
...
void spiInit(const uint8_t sckPin, const uint8_t misoPin, const uint8_t mosiPin)
{
    ...
    pDev->miso = misoPin;
    pDev->mosi = mosiPin;
    pDev->dev = ((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x3000));
    pDev->af = ((uint8_t)0x05);
...
# 229 "stm/src/spi.c"
    ...
    IOConfigGPIOAF(
            IOGetByTag(spi->sck),
            ((GPIO_Mode_AF) | ((GPIO_Fast_Speed) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_DOWN) << 5)),
            spi->af);

    IOConfigGPIOAF(
            IOGetByTag(spi->miso),
            ((GPIO_Mode_AF) | ((GPIO_Fast_Speed) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_UP) << 5)),
            spi->af);

    IOConfigGPIOAF(
            IOGetByTag(spi->mosi),
            ((GPIO_Mode_AF) | ((GPIO_Fast_Speed) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_NOPULL) << 5)),
            spi->af);


    SPI_I2S_DeInit(spi->dev);

    SPI_I2S_DMACmd(spi->dev, ((uint16_t)0x0002) | ((uint16_t)0x0001), DISABLE);
...
static void _spiSetBusInstance(spiDevice_t *dev, const uint8_t csPin)
{
    ...
    IOInit(dev->csnPin, OWNER_GYRO_CS, (0 + 1));

    IOConfigGPIO(dev->csnPin,
            ((GPIO_Mode_OUT) | ((GPIO_Fast_Speed) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_NOPULL) << 5)));
